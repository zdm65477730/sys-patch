        ...
        if (does_file_exist(LOG_PATH)) {
            ...
            ini_browse([](const mTCHAR *Section, const mTCHAR *Key, const mTCHAR *Value, void *UserData){
                ...
                if (value.starts_with("Patched")) {
                    ...
                    item->setValue("PatchedGuiLogListItemText"_tr, true);
                    user->list->addItem(item);
                } else if (value.starts_with("Unpatched")) {
                    auto *item = new tsl::elm::ListItem(Key);
                    item->setValue("UnPatchedGuiLogListItemText"_tr, true);
                    user->list->addItem(item);
                } else if (value.starts_with("Disabled")) {
                    auto *item = new tsl::elm::ListItem(Key);
                    item->setValue("DisabledGuiLogListItemText"_tr, true);
                    user->list->addItem(item);
                } else if (user->last_section == "stats") {
                    auto *item = new tsl::elm::ListItem(map_stats_to_text(Key));
